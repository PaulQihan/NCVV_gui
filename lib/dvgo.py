...
import ipdb
from .lowrank_volume import  TensorDVGO,TensorCP,TensorVP,TensorPCA,TensorDCT,TensorDVGORes,TensorDVGODeform
...
class DirectVoxGO(torch.nn.Module):
    ...
    def load_pretrain_deform_res(self, filepath, deform_res_stage="res",deform_low_reso=False):
        if deform_res_stage=="res":
            ...
            if self.cfg.density_deform:
                ...
                self.former_density = self.grid_sampler(deform_xyz, density).unsqueeze(-1).permute(3, 0, 1, 2).unsqueeze(0)
                self.former_density_cur = F.interpolate(density, size=tuple(self.world_size), mode='trilinear', align_corners=True)
            self.k0.former_k0_cur = torch.tensor(
                F.interpolate(self.k0.former_k0, size=tuple(self.world_size), mode='trilinear', align_corners=True),
                requires_grad=False)
    ...
    def scale_volume_grid(self, num_voxels):
        ...
        if self.use_deform:
            ...
            self.former_density_cur= F.interpolate(self.former_density, size=tuple(self.world_size), mode='trilinear', align_corners=True)
            self.density = torch.nn.Parameter(
                F.interpolate(self.density.data, size=tuple(self.world_size), mode='trilinear',
                              align_corners=True))
        ...
        if self.use_res:
            self_alpha = F.max_pool3d(self.activate_density(self.density+self.former_density_cur), kernel_size=3, padding=1, stride=1)[0,0]
        else:
    ...
    def sample_ray(self, rays_o, rays_d, near, far, stepsize, is_train=False, **render_kwargs):
        ...
        # # print(f'------- mask_outbbox : ', mask_outbbox)
        # print(f'(xyz_min, xyz_max)   : ', self.xyz_min, self.xyz_max)
        # exit(0)
    ...
    def forward_pts(self, pts):
        density = self.grid_sampler(pts, self.density)
        density = density + self.act_shift

         # query for color
        if not self.rgbnet_full_implicit:
            k0 = self.grid_sampler(pts, self.k0)

        k0_diffuse = None
        if not self.rgbnet_direct:
            k0_diffuse = k0[:, :3]
            
        return density,k0_diffuse
    '''
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        if self.use_res:
            density = self.grid_sampler(ray_pts, self.density+self.former_density_cur)
        else:
        ...
        if not self.rgbnet_full_implicit:
            ...
            #    vp = self.grid_sampler_new(ray_pts, self.vp)
        ...
        if self.rgbnet is None:
            ...
            #rgb_feat_vp = torch.cat([vp, viewdirs_emb], -1)
      
            rgb_logit = self.rgbnet(rgb_feat)

            #rgb_vp_logit = self.localnet(rgb_feat_vp)

            if self.rgbnet_direct:
                rgb = torch.sigmoid(rgb_logit)
                '''
                rgb_vp = torch.sigmoid(rgb_vp_logit)


                rgb_marched_raw = segment_coo(
                        src=(weights.unsqueeze(-1).detach() * rgb_vp),
                        index=ray_id,
                        out=torch.zeros([N, 3]),
                        reduce='sum')
                rgb_marched_raw += (alphainv_last.unsqueeze(-1).detach() * render_kwargs['bg']).detach()

                ret_dict.update({'rgb_marched_raw':rgb_marched_raw})
                '''
            else:
...
class MaskCache(nn.Module):
    def __init__(self, path=None, mask_cache_thres=None, mask=None, xyz_min=None, xyz_max=None):
        super().__init__()
        #print(path) path就是coarse的tar path
