...
@torch.no_grad()
def render_viewpoint_frame(model, cfg,render_pose, HW, K, frame_id,ndc, render_kwargs,
                      gt_imgs=None, savedir=None, render_factor=0,
                      eval_ssim=False, eval_lpips_alex=False, eval_lpips_vgg=False, model_callback=None,mute=False, return_time=False):
    '''Render images for the given viewpoints; run evaluation if gt given.
    '''
    ...
    c2w = render_pose
    model, render_kwargs = model_callback(model, render_kwargs, frame_id)
    ...
    # with Timer("render_image : "):
    render_result_chunks = [
        {k: v for k, v in model(ro, rd, vd, **render_kwargs).items() if k in keys}
        # for ro, rd, vd in zip(rays_o.split(8192*128, 0), rays_d.split(8192*128, 0), viewdirs.split(8192*128, 0))
        for ro, rd, vd in zip(rays_o.split(8192*128, 0), rays_d.split(8192*128, 0), viewdirs.split(8192*128, 0))
        # for ro, rd, vd in zip(rays_o.split(8192*64, 0), rays_d.split(8192*64, 0), viewdirs.split(8192*64, 0))
        # for ro, rd, vd in zip(rays_o.split(8192*16, 0), rays_d.split(8192*16, 0), viewdirs.split(8192*16, 0))
    ]
    ...
    if savedir is not None:
        ...
        rgb8 = utils.to8b(rgb)
        filename = os.path.join(savedir, '{:03d}.jpg'.format(frame_id))
        imageio.imwrite(filename, rgb8)
        depth8 = utils.to8b(1 - depth / np.max(depth))
        filename = os.path.join(savedir, '{:03d}_depth.jpg'.format(frame_id))
        imageio.imwrite(filename, depth8)

    if gt_imgs is not None and render_factor == 0:
        p = -10. * np.log10(np.mean(np.square(rgb - gt_imgs[frame_id])))
        psnrs.append(p)
        if eval_ssim:
            ssims.append(utils.rgb_ssim(rgb, gt_imgs[frame_id], max_val=1))
        if eval_lpips_alex:
            lpips_alex.append(utils.rgb_lpips(rgb, gt_imgs[frame_id], net_name='alex', device=c2w.device))
        if eval_lpips_vgg:
            lpips_vgg.append(utils.rgb_lpips(rgb, gt_imgs[frame_id], net_name='vgg', device=c2w.device))
