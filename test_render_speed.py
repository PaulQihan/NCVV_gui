...
class ncvv:
    ...
    def decode(self):
        ...
        while(True):
            # t = time.time()
            self.length = download_chunks(self.download_path, self.frame_count, self.file_path)/1000
            ...
            count += 1
            if count == 50:
                print("entropy average: ", entropy_sum/count)
                print("quant average: ", quant_sum/count)
                print("idct average: ", idct_sum/count)
                print("other average: ", decode_sum/count-entropy_sum/count-quant_sum/count-idct_sum/count)
                print("recover average: ", recover_sum/count)
    ...
    def render(self):
        ...
        # exit(0)
        camera_K = torch.tensor(self.sK).float()
        camera_K = camera_K /2
        camera_K[-1,-1] = 1

        total_costs = [] 
        for i in range(500):
            render_pose = torch.tensor(self.render_poses[i%self.render_poses.shape[0]]).float().cuda()
        
            frame_id = i%200
            with torch.cuda.amp.autocast(enabled=self.args.fp16):
                render_time = render_viewpoint_frame(
                    cfg=self.cfg,
            ...
            total_costs.append(render_time)
        total_costs = np.array(total_costs).mean()
        print(f'-------------------- average_render time ({self.HW[0]},{self.HW[1]}) : ', total_costs)
