...
def encode_huffman(value, layer_type):
    ...
    run, nonzero = value
        ...
        # )
    if nonzero ==0:
        raise ValueError (f'AC coefficient nonzero {value} should be not be 0')
    elif nonzero<=-1024:
        print(f'AC coefficient nonzero {value} should be within [-1023, 0)')
        nonzero=-1023
    elif nonzero>=1024:
        print(f'AC coefficient nonzero {value} should be within (0, 1023]')
        nonzero = 1023
