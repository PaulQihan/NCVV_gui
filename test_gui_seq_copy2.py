...
from streamer.streamer import download_url, download_chunks
...
def config_parser():
    ...
    parser.add_argument("--url_path", type=str, default='http://127.0.0.1:80/xzq_wmyparams_deform_tv_res_cube_l1/dynamic+99',
                        help='url path')    
    ...
    parser.add_argument("--group_size", type=int, default=100000000,
                       help='key frame cover how many frames')

    parser.add_argument('--local_ip', type=str, default="10.19.127.3")
...
class ncvv:
    def __init__(self):
        ...
        self.download_path=self.args.url_path
        ...
        self.length = 0
        # self.length = download_chunks(self.download_path, 0, self.file_path)/1000

        self.jsonfile = os.path.join(self.file_path, f'model_kwargs.json')
        download_url(os.path.join(self.download_path, f'model_kwargs.json'), self.jsonfile)
        ...
        # print(os.path.join(self.file_path, f'model_kwargs.json'))
        # print('---- xyz_min, xyz_max || ', self.model_kwargs['xyz_min'], self.model_kwargs['xyz_max'])
        # exit(0)
        ...
        self.url_path = self.args.url_path

        if self.args.gui:
            self.total_frames = self.args.render_360
            self.decoder_fixed_fps = 20 

            from gui_module.gui_v2 import NeRFGUI
            render_func = self.gui_render_func
            gui = NeRFGUI(self.args, render_func=render_func, \
                max_frame=self.total_frames, start_frame=self.args.render_start_frame,
                decoder_fps=self.decoder_fixed_fps)
            gui.render()
        else:
    ...
    def decode(self):
        self.frame_count = int(self.frame_id)
        print(f"first frame {self.frame_count}")
        entropy_sum = 0.0
        quant_sum = 0.0
        idct_sum = 0.0
        recover_sum = 0.0
        decode_sum = 0.0
        count = 0
        while(self.frame_count <= self.frame_id):
            self.jsonfile = os.path.join(self.file_path,f'newheader_{self.frame_id}.json')
            print(self.frame_count, " ", self.frame_id )
            ...
            t = time.time()
            self.residual_rec_dct, entropy_time, quant_time, idct_time = decode_jpeg_huffman(os.path.join(self.file_path, f'2new_{self.frame_count}.ncrf'), self.header, device=self.device)
            #print("entropy time: ", entropy_time)
            #print("quant time: ", quant_time)
            #print("idct time: ", idct_time)
            entropy_sum += entropy_time
            quant_sum += quant_time
            idct_sum += idct_time
            decode_time = time.time() - t
            decode_sum += decode_time

            if self.frame_count%self.gop==0:
                ...
                recover_time = time.time() - t
                recover_sum += recover_time
            else:
            ...
            count += 1
            if count == 500:
                print("entropy average: ", entropy_sum/500)
                print("quant average: ", quant_sum/500)
                print("idct average: ", idct_sum/500)
                print("other average: ", decode_sum/500-entropy_sum/500-quant_sum/500-idct_sum/500)
                print("recover average: ", recover_sum/500)
    ...
    def model_callback(self, model, render_kwargs, frame_id):
            if frame_id != self.args.render_start_frame and frame_id != self.frame_id:
                self.model_receive = next(self.mmap_iter)
                self.dvgo_model.density = torch.nn.Parameter(self.model_receive[:, :1])
                self.dvgo_model.k0.k0 = torch.nn.Parameter(self.model_receive[:, 1:])
                if self.gop < 100000000:
                    i_frame = (frame_id // self.gop % self.gop) * self.gop          
                    print('-------------- iiii frame_id : ', i_frame)
                    print('-------------- frame_id : ', frame_id)
                    if abs(self.frame_id - frame_id) > 3 and i_frame % self.gop == 0:
                        tmp_frame_id = i_frame
                        # self.length = download_chunks(self.download_path, tmp_frame_id, self.file_path)/1000
                        self.frame_id = tmp_frame_id
                        print("download ", self.frame_id)
                        
                        self.mmap_iter=self.decode()
                    elif self.total_frames < self.frame_id - 1:
                        tmp_frame_id = 0
                        t = time.time()
                        # self.length = download_chunks(self.download_path, tmp_frame_id, self.file_path)/1000 
                        self.frame_id = tmp_frame_id                    
                        ...
                        tmp_frame_id = self.frame_id + 1
                        t = time.time()
                        print("download ", tmp_frame_id)
                        # self.length = download_chunks(self.download_path, tmp_frame_id, self.file_path)/1000  
                        self.frame_id = tmp_frame_id                   
                else:
    ...
    def render(self):
        ...
        render_pose = torch.tensor(self.render_poses[50]).float().cuda()

        # self.render_viewpoints_kwargs['render_kwargs']['far'] = self.render_viewpoints_kwargs['render_kwargs']['far'] * 5
        # near = self.render_viewpoints_kwargs['render_kwargs']['near']
        # far = self.render_viewpoints_kwargs['render_kwargs']['far']
    ...
    def gui_render_func(self,
        ...
        # render_pose = self.sT
        # K = self.sK
        # self.cfg.frame_num = 92
        # self.total_frames = 91
        frame_id = int(time * self.decoder_fixed_fps)
        ...
        #TODO: input bitrate_1, bitrate_2 here
        outputs['bitrate'] = self.length
        outputs['ip'] = self.local_ip 
        outputs['url'] = self.url_path

        #TODO:
        '''
        app_to_decoder()-> times
        app_to_decoder()-> camera_pose
        app_display_bitrate(b1, b2)
        '''
