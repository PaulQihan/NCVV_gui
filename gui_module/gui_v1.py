...
class NeRFGUI:
    ...
    def test_step(self):
        ...
        if self.need_update or self.spp < self.opt.max_spp:
            ...
            outputs  = self.render_func(self.cam.pose, self.cam.intrinsics, self.W, self.H, self.frame_id)
    ...
    def register_dpg(self):
        ...
        with dpg.window(label="Control", tag="_control_window", width=400, height=300):
            ...
            with dpg.collapsing_header(label="Options", default_open=True):
                ...
                # frame slider
                def callback_set_frame(sender, app_data):
                    self.frame_id = app_data
                    self.need_update = True

                dpg.add_slider_int(label="frame", min_value=0, max_value=self.max_frame, format="%d", default_value=self.frame_id, callback=callback_set_frame)
