...
Descripttion: 
version: 
Author: aqiang <heqh1@shanghaitech.edu.cn>
Date: 2022-09-02 22:57:58
LastEditors: Please set LastEditors
LastEditTime: 2022-05-12 13:26:17
'''

'''
传输需要做出的一些改变：
decode的时候第1帧 会有许多参数,和rgbnet
其他每一帧还有header file, mask, motion vector 等其他文件
'''
...
from codec import  decode_jpeg_mmap,Timer,recover_misc
import torch
...
def mmap_decode():
    with open(args.mmap_path, "r+b") as f:
        ...
        while(True):
            ...
            if data[0:1] > 0:
                ...
                size = int(np.frombuffer(mm[4:8], dtype=np.uint32)[0:1])
                packet=bitarray(buffer=mm[8:size+7])
                jsonfile=os.path.join(file_path,f'header_{frame_count}.json')
                with open(jsonfile) as f:
                    header = json.load(f)
                ...
                print(f"decode frame {frame_count}, size {size}")
                with Timer("Decode", device):
                    residual_rec_dct = decode_jpeg_mmap(packet, header, device=device)

                if frame_count==0:
                    former_rec=torch.zeros(( mask.size(0), n_channel, voxel_size**3),device=device) #big_data_0
                    former_rec[:, 0, :] = former_rec[:, 0, :] - 4.1

                model,former_rec=recover_misc(residual_rec_dct,former_rec,header, mask ,n_channel=n_channel,device=device)
